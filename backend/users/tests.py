...
class EmailVerificationCodeFlowTests(TestCase):
    ...
    def test_send_verification_code_returns_demo_code_when_smtp_fails_in_demo_mode(
        ...
        verification_code = EmailVerificationCode.objects.get(email="new@example.com")
        self.assertEqual(response.json()["verification_code"], verification_code.code)
    ...
    def test_send_verification_code_returns_503_when_smtp_fails_without_demo_mode(
        ...
        self.assertEqual(
            response.json()["detail"], "Сервис отправки email временно недоступен"
        )
        self.assertTrue(
            EmailVerificationCode.objects.filter(email="new@example.com").exists()
        )
